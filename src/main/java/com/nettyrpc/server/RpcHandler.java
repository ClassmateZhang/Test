...
public class RpcHandler extends SimpleChannelInboundHandler<RpcRequest> {
    ...
    public void channelRead0(final ChannelHandlerContext ctx,final RpcRequest request) throws Exception {
        RpcServer.submit(new Runnable() {
            @Override
            public void run() {
                System.out.println("[服务端收到客户端发来的信息]"+request);
                logger.info("Receive request " + request.getRequestId());
                logger.info("ClassName==>"+request.getClassName()+"  MethodName==>"+request.getMethodName());
    ...
    public void channelActive(ChannelHandlerContext ctx) throws Exception {
        ...
        logger.info("[有一个链接" + ctx.channel().remoteAddress() + "连接到服务器]");
    }
}
